...
async function getChannelData(channelId: string, apiKey: string) {
  ...
  const data = await requestJson<{ items: Array<{ snippet: { title: string }; statistics: { subscriberCount?: string } }> }>(url);
  ...
  return {
    ...
    subscriberCount: result.statistics.subscriberCount ?? '0',
  };
}
